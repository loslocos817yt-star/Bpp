...
def compilar_proyecto(nombre_archivo):
    ...
    os.system('clear')
    print(f"--- EXPORTAR/COMPILAR: {nombre_archivo} ---")
    print("[1] Python (.py) | [2] C++ (.cpp) | [3] BINARIO (Ejecutable)")
    opc = input("\nSelección: ").strip()
    ...
    ruta_temp_cpp = ruta_base + "_temp.cpp"

    try:
        if opc in ["2", "3"]:
            with open(ruta_temp_cpp if opc == "3" else ruta_final, "w", encoding="utf-8") as f:
                ...
                for l in lineas:
                    ...
                    if l.startswith("val "):
                        ...
                        if m:
                            var, val = m.groups()
                            f.write(f'    string {var} = {val};\n')
                            memoria_vars.add(var)
                    elif l.startswith("text"):
                        res = re.search(r'\((.*)\)', l.split("->")[0])
                        if res: f.write(f'    cout << {res.group(1)} << endl;\n')
                    elif l.startswith("esperar"):
                        res = re.search(r'\((.*)\)', l)
                        if res: f.write(f'    this_thread::sleep_for(chrono::milliseconds((int)({res.group(1)}*1000)));\n')
                    elif l == "limpiar":
            ...
            if opc == "3":
                print("\n[Compilando con G++...]")
                proceso = subprocess.run(["g++", ruta_temp_cpp, "-o", ruta_final], capture_output=True, text=True)
                if proceso.returncode == 0:
                    print(f"\n[OK]: Binario generado en {ruta_final}")
                    print(f"Pruébalo con: ./{ruta_final}")
                    os.remove(ruta_temp_cpp) 
                else:
                    print(f"\n[ERROR DE COMPILACIÓN]:\n{proceso.stderr}")
            else:
                print(f"\n[OK]: Código C++ generado en {ruta_final}")
            ...
            print(f"\n[OK]: Script Python generado en {ruta_final}")
...
def menu_proyectos():
    while True:
        ...
        opcion = input("\n[N] Nuevo | [B] Borrar | [W] Compilar/Exportar | [X] Salir\n> ").strip().upper()
        if opcion == "N":
