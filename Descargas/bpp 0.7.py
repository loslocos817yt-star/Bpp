...
import sys, re, os, time, subprocess, random
...
def ejecutar_comando(linea, memoria, num_linea=0):
    ...
    try:
        ...
        # --- NUEVO COMANDO: ALEATORIA ---
        elif l_clean.startswith("aleatoria"):
            match = re.search(r'aleatoria\(\s*(\d+)\s*,\s*(\d+)\s*\)\s*->\s*(\w+)', l_clean)
            if match:
                vmin, vmax, var_dest = match.groups()
                memoria[var_dest] = str(random.randint(int(vmin), int(vmax)))
            else: return error_fatal("SINTAXIS", "Uso: aleatoria(min, max) -> variable")
...
def compilar_proyecto(nombre_archivo):
    ...
    os.system('clear' if sys.platform != 'win32' else 'cls')
    print(f"\033[94m--- B++ COMPILER CORE v0.7 ---\033[0m")
    ...
    try:
        ...
        if opc in ["2", "3"]:
            ...
            with open(temp_cpp, "w") as f:
                ...
                f.write("int main() {\n    // Generado por B++ Core v0.7\n    srand(time(NULL));\n")
                for l in lineas:
                    l_clean = l.strip()
                    if l_clean.startswith("val "):
                        m = re.match(r'val\s+(\w+)\s*=\s*(.*)', l_clean)
                        if m: f.write(f'    string {m.group(1)} = {m.group(2)};\n')
                    elif l_clean.startswith("text"):
                        res = re.search(r'\((.*)\)', l_clean.split("->")[0])
                        if res: f.write(f'    cout << {res.group(1)} << endl;\n')
                    elif l_clean.startswith("limpiar"): f.write('    system("clear");\n')
                    elif l_clean.startswith("aleatoria"):
                        m = re.search(r'aleatoria\(\s*(\d+)\s*,\s*(\d+)\s*\)\s*->\s*(\w+)', l_clean)
                        if m: f.write(f'    string {m.group(3)} = to_string({m.group(1)} + rand() % ({m.group(2)} - {m.group(1)} + 1));\n')
                    elif l_clean.startswith("Repetir"):
                        if "infinito" in l_clean.lower(): f.write("    while(true) {\n")
                        else:
                            m_rep = re.search(r'Repetir\s+(\d+)', l_clean)
                            if m_rep: f.write(f"    for(int _i=0; _i<{m_rep.group(1)}; _i++) {{\n")
                    elif l_clean == "FB":
                        f.write("    }\n")
                f.write("    return 0;\n}")
            ...
            with open(ruta_final, "w") as f:
                f.write("import time, os, sys, random\n\n# B++ SDK v0.7 Generated Code\n")
                ...
                for l in lineas:
                    ...
                    if "text" in l_clean:
                        res = re.search(r'\((.*)\)', l_clean.split("->")[0])
                        if res: f.write(f"{prefix}print({res.group(1)})\n")
                    elif "val " in l_clean:
                        m = re.match(r'val\s+(\w+)\s*=\s*(.*)', l_clean)
                        if m: f.write(f"{prefix}memoria['{m.group(1)}'] = {m.group(2)}\n")
                    elif l_clean.startswith("aleatoria"):
                        m = re.search(r'aleatoria\(\s*(\d+)\s*,\s*(\d+)\s*\)\s*->\s*(\w+)', l_clean)
                        if m: f.write(f"{prefix}memoria['{m.group(3)}'] = str(random.randint({m.group(1)}, {m.group(2)}))\n")
                    elif l_clean.startswith("Repetir"):
                        if "infinito" in l_clean.lower(): f.write(f"{prefix}while True:\n")
                        else:
                            m_rep = re.search(r'Repetir\s+(\d+)', l_clean)
                            if m_rep: f.write(f"{prefix}for _ in range({m_rep.group(1)}):\n")
                        nivel_tabs += 1
                    elif l_clean == "FB":
                        nivel_tabs = max(0, nivel_tabs - 1)
                        if nivel_tabs == 0: prefix = ""
...
def menu_proyectos():
    while True:
        ...
        print(f"   B++ PROJECT MANAGER v0.7")
    ...
    while True:
        ...
        if cmd == "RUN":
            ...
            while i < len(lineas):
                ...
                # Interceptar sistema de bucles
                if l_clean.startswith("Repetir"):
                    fin_bucle = -1
                    nivel_rep = 0
                    # Buscar el cierre correspondiente
                    for j in range(i + 1, len(lineas)):
                        if lineas[j].strip().startswith("Repetir"):
                            nivel_rep += 1
                        elif lineas[j].strip() == "FB":
                            if nivel_rep == 0:
                                fin_bucle = j
                                break
                            else:
                    ...
                    if fin_bucle == -1:
                        print(f"\033[91mB++ ERROR: Bucle 'Repetir' en línea {i+1} no tiene 'FB' de cierre.\033[0m")
                        break
                        
                    if "infinito" in l_clean.lower():
                        iters = -1
                    else:
                        match = re.search(r'Repetir\s+(\d+)', l_clean)
                        iters = int(match.group(1)) if match else 0
                        
                    if iters != 0:
                        pila_bucles.append({'inicio': i + 1, 'fin': fin_bucle, 'iters': iters})
                    else:
                        i = fin_bucle # Si pide repetir 0 veces, lo saltamos
                    i += 1
                    continue
                    
                elif l_clean == "FB":
                    if pila_bucles:
                        bucle_actual = pila_bucles[-1]
                        if bucle_actual['iters'] > 0:
                            bucle_actual['iters'] -= 1
                            
                        if bucle_actual['iters'] == 0:
                            pila_bucles.pop()
                            i += 1
                        else:
                            i = bucle_actual['inicio']
                    else:
                        i += 1
                    continue
